...
func (s *EchoServer) AddCustomer(ctx echo.Context) error {
	...
	//save customer to DB
	savedCustomer, err := s.DB.AddCustomer(ctx.Request().Context(), customer)
	if err != nil {
		switch err.(type) {
	...
	return ctx.JSON(http.StatusCreated, savedCustomer)
...
func (s *EchoServer) UpdateCustomer(ctx echo.Context) error {
	...
	if ID != customer.CustomerID {
		return ctx.JSON(http.StatusBadRequest, "id on path != customerId in payload")
	}
